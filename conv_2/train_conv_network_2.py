...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    # Variables to hold parameters of convolution 0
    conv0_weights = tf.Variable(
        tf.truncated_normal(
            [5, 5, NUM_CHANNELS, 32], stddev=0.1, seed=SEED  # 5x5 filter, depth 32.
        )
    )
    conv0_biases = tf.Variable(tf.zeros([32]))
    ...
    conv1_weights = tf.Variable(
        tf.truncated_normal(
            [5, 5, 32, 32], stddev=0.1, seed=SEED  # 5x5 filter, depth 32.
        )
    )
    ...
    def model(data, train=False, return_means_and_variances=False):
        ...
        conv0 = tf.nn.conv2d(data, conv0_weights, strides=[1, 1, 1, 1], padding="SAME")
        relu0 = tf.nn.relu(tf.nn.bias_add(conv0, conv0_biases))
        # 2D convolution, with 'SAME' padding (i.e. the output feature map has
        # the same size as the input). Note that {strides} is a 4D array whose
        # shape matches the data layout: [image index, y, x, depth].
        #conv = tf.nn.conv2d(data, conv1_weights, strides=[1, 1, 1, 1], padding="SAME")
        conv = tf.nn.conv2d(relu0, conv1_weights, strides=[1, 1, 1, 1], padding="SAME")
        # Batch normalisation
